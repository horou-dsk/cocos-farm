import { _decorator, Component, Node, Vec3, Vec2, Sprite, tween, Label, Quat } from 'cc';
...
export class PlantEffect extends Component {
    ...
    reap() {
        ...
        const vertical = subResult.clone().cross(new Vec3(subResult.x, subResult.y, -1)).normalize();
        ...
        tween(this.node.getWorldScale()).to(0.4, new Vec3(0.2, 0.2, 0), {
            easing: 'expoInOut',
    ...
    update(deltaTime: number) {
        if (this.running) {
            ...
            const scale = new Vec2(this.scale.x - (this.t * 0.5), this.scale.y - (this.t * 0.5));
